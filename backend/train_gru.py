"""
train_gru.py — Train a stacked GRU for Smart Energy Consumption Forecasting.

Architecture
------------
GRU(128, return_sequences=True, dropout=0.2, recurrent_dropout=0.1)
-> GRU(64, return_sequences=False, dropout=0.2)
-> Dense(64, relu) -> Dropout(0.25) -> Dense(32, relu) -> Dense(1)
...
MODEL_OUT = os.path.join(MODEL_DIR, "gru_model.keras")
PLOT_OUT  = os.path.join(PLOT_DIR,  "gru_history.png")


# ---------------------------------------------------------------------------
# Model builder
# ---------------------------------------------------------------------------
def build_gru(input_shape: tuple) -> Model:
    """
    Parameters
    ----------
    ...
    model = Model(inputs=inp, outputs=out, name="GRU_EnergyForecaster")
    model.compile(
